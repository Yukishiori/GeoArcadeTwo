...
public class ExplosionSquare extends GameObject{
    ...
    public void config() {
        for (double i = 0; i <= 360 ; i += 45 ) {
            ...
            explosionParticleSquare.velocity.set((new Vector2D(0,1)).rotate(i).multiply(random.nextInt(7)+4));
            explosionParticleSquare.frameCounter = new FrameCounter(random.nextInt(1) + 10);
        }
    }
}
